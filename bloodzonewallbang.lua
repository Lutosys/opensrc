...
local config = {
    wallbang = true -- WALLBANG IS RISKY ( server kicks you if you kill like 10 people in 2 secodns through walls )
} :: {wallbang : boolean}
...
local function GetClosestPlayer(): Instance?
    ...
    for _, v in pairs(plrs) do
        ...
        if char:GetAttribute("Immune") and char:GetAttribute("Immune") == true then continue end
...
local hookSuccess, err = pcall(function()
    old = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        local args = {...}
        if tostring(self) == "GunHit" and getnamecallmethod() == "FireServer" then
            if target and typeof(target) == "Instance" then
                local targetPart = target :: any
                if rawget(args[2][1], "Position") and rawget(args[2][1], "Instance") then
